...
`id`, `chat_id`, `contact_id`, `instance_name`, `remote_jid`, `contact_name`, `phone`, `trigger_msg_id`, `matched_phrase`, `fragment_start_at`, `fragment_end_at`, `transcript_snapshot`, `messages_snapshot` (JSON estruturado do mesmo trecho — o front renderiza daqui), `temperatura_snapshot`, `status` (`pendente`/`aprovado`/`descartado`/`enviado`), `sent_to_radar`, `notion_page_id`, `encaminhamento`, `sent_at`, `created_at`, `updated_at`. `unique(chat_id, trigger_msg_id)` = idempotência.

- **Detecção** (`radar_pe_detect_cases_for_chat`/`radar_pe_detect_cases`): mensagem **nossa** cujo body contém uma frase ativa → abre caso `pendente`, congelando as últimas **25** mensagens + o gatilho (`transcript_snapshot` em texto e `messages_snapshot` em JSON). Roda no fim de `radar_pe_upsert_chat` (sempre fresca); o bulk cobre o histórico. Os casos criados antes da coluna JSON são preenchidos por `radar_pe_backfill_case_messages()`.
...
- **Recorte de revisão** (front): enquanto o caso está `pendente`, o drawer e o form montam um **recorte por pessoa (telefone)**, atravessando as sessões/instâncias do mesmo contato (janela: 30 dias antes do gatilho + tudo depois, cap 60+60, com rótulo da sessão de origem). Ao aprovar/descartar, o recorte visto é **congelado** em `messages_snapshot`/`transcript_snapshot` — assim o registro reflete o que o time realmente leu, incluindo o que chegou **depois** do gatilho.
- Cada caso congela um trecho no momento da decisão; conversa continuar ⇒ novos casos, nunca reescrever o antigo. `trigger_msg_id` registra qual mensagem disparou.
